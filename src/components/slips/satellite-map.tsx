...
export function SatelliteDockDetection() {
  ...
  const queryOSMForMarina = async (lat: number, lng: number): Promise<DetectedDock[] | null> => {
    try {
      ...
      for (const dock of docks) {
        ...
        if (!wasMerged) mergedDocks.push({ ...dock, slipStatuses: generateSlipStatuses(dock.slipCount) });
      }

      // Sort by position (north to south) and rename
      mergedDocks.sort((a, b) => b.lat - a.lat);
      mergedDocks.forEach((d, i) => {
        d.name = `Dock ${String.fromCharCode(65 + i)}`;
      });

      toast.success(`Found ${mergedDocks.length} docks from OpenStreetMap${marinaName ? ` for ${marinaName}` : ""}`);
      return mergedDocks;
    } catch (err) {
  ...
  const renderDockOverlay = () => {
    ...
    return docks.map((dock) => {
      ...
      const pxPerMeterLng = pxDeltaLng ? (pxDeltaLng.x - center.x) : 1;
      const pxPerMeterLat = pxDeltaLat ? (pxDeltaLat.y - center.y) : 1;
      const halfWPx = (dock.width / 2) * pxPerMeterLng;
      const halfHPx = (dock.height / 2) * pxPerMeterLat;
